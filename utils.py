...
def get_label_info(csv_path):
    """
    Retrieves label information from a CSV file.

    Args:
        csv_path (str): The path to the CSV file.

    Returns:
        dict: A dictionary containing label information, where the keys are label names and the values are lists
              containing the red, green, blue values, image class (from GTA Dataset), and csId (from Cityscape Dataset)
    """
...
def colored_image_to_segmentation(label, label_info):
    """
    Convert a colored image to a segmentation map.

    Args:
        label (numpy.ndarray): The colored image with shape [H, W, 3].
        label_info (dict): A dictionary containing label information.
    ...
    # la label non deve essere one-hot encoded per andare nella loss
    # preso da forum pytorch: if you are passing class indices, make sure that the target contains the class indices
    # ( not one-hot encoded) as a LongTensor and thus does not use a “channel / class ” dimension
...
def one_hot_to_segmentation(semantic_map, num_classes=20):
    """
    Converts a one-hot encoded semantic map to a segmentation map.

    Args:
        semantic_map (numpy.ndarray): The one-hot encoded semantic map. Shape: [H, W]
        num_classes (int): The number of classes in the semantic map. Default: 20

    Returns:
        torch.Tensor: The segmentation map. Shape: [class_num, H, W]
    """
    ...
    semantic_map = semantic_map[:, :, :-1]  # remove void class from [H, W, 20] to [H, W, 19]
...
def cs_id_to_class(label, label_info):
    """
    Convert an image of the labels, where the pixels have values corresponding 
    to the Cityscapes IDs, into a semantic map, where the pixels have values
    corresponding to the class indices.
    
    Args:
        label (numpy.ndarray): The label map with class IDs. Shape: [H, W]
        label_info (dict): A dictionary containing class information.

    Returns:
        numpy.ndarray: The semantic map with class indices. Shape: [H, W]
    """
